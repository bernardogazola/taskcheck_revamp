...
export async function createReportAction(
  ...
  try {
    ...
    // Check if user is authenticated and is an aluno
    if (!session?.user) {
    ...
    const texto_reflexao = formData.get("texto_reflexao") as string;
    ...
    // Validate data
    const validationResult = relatorioSchema.safeParse(reportData);
    ...
    if (!category) {
      return {
        ...
        error: {
          ...
          details: {
            id_categoria: ["Categoria inválida ou não encontrada"],
          },
        },
      };
    }

    // Read file as buffer for storage
    const arrayBuffer = await certificadoFile.arrayBuffer();
    const buffer = Buffer.from(arrayBuffer);

    // Create report
    await prisma.relatorioAtividade.create({
      data: {
        nome,
        texto_reflexao,
        data_realizacao: new Date(data_realizacao),
        data_envio: new Date(),
        status: StatusRelatorio.AGUARDANDO_VALIDACAO,
        horas_validadas: 0, // Initially 0, will be set by professor when validating
        certificado: buffer,
        id_aluno: userId,
        id_categoria,
      },
